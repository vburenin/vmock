...
class TestVmockMatchers(unittest.TestCase):
    ...
    def test_restore_one_func(self):
        ...
        self.v.mock_method(sc, 'simple_func')
    ...
    def test_restore_two_funcs(self):
        ...
        self.v.mock_method(sc, 'simple_func')
        self.v.mock_method(sc, 'func_with_one_arg')
        self.assertNotEqual(orig_simple_func, sc.simple_func)
    ...
    def test_func_stub_anytimes(self):
        self.v.mock_method(sc, 'simple_func')().returns(1111).anyorder()
        self.v.replay()
    ...
    def test_func_stub_5_times(self):
        self.v.mock_method(sc, 'simple_func')().returns(11).anyorder().times(5)
        self.v.replay()

        for _ in range(6):
            self.assertEqual(11, sc.simple_func())
        self.assertRaises(mockerrors.CallsNumberError, self.v.verify)

    def test_func_mock_3_times(self):
        self.v.mock_method(sc, 'simple_func')().returns(1111).times(3)
        self.v.replay()
    ...
    def test_func_mock_3_times_call_only_2_times(self):
        ...
        self.assertRaises(mockerrors.CallSequenceError, self.v.verify)

    def test_static_mock_any_times(self):
        self.v.stub_method(sc, 'simple_func')().returns(2222)
        for _ in range(111):
    ...
    def test_static_mock_10_times_but_called_11_times(self):
        self.v.stub_method(sc, 'simple_func')().returns(2222).times(10)
        for _ in range(11):
            self.assertEqual(2222, sc.simple_func())
        self.v.replay()
        self.assertRaises(mockerrors.CallsNumberError, self.v.verify)
    ...
    def test_property_mock(self):
        m_class = self.v.mock_class(sc.SimpleClass, display_name='mcls')
    ...
    def test_mock_whole_class(self):
        m_class = self.v.mock_class(sc.SimpleClass)
    ...
    def test_funcs_mock_different_sequence(self):
        f1 = self.v.mock_method(sc, 'simple_func')
        f2 = self.v.mock_method(sc, 'func_with_one_arg')
        f1().returns(1)
    ...
    def test_mock_does(self):
        ...
        f1 = self.v.stub_method(sc, 'simple_func')
        f1().does(does_func)
    ...
    def test_stub_does_args(self):
        ...
        f1 = self.v.stub_method(sc, 'func_with_one_arg_and_many_other_kwargs')
    ...
    def test_mock_does_args(self):
        ...
        f1 = self.v.mock_method(sc, 'func_with_one_arg_and_many_other_kwargs')
        f1(1, b=2).does(does_func)
    ...
    def test_ctor_and_class_mock(self):
        m_class = self.v.mock_class(sc.SimpleClass)
        self.v.mock_constructor(sc, 'SimpleClass')(1, 2).returns(m_class)
    ...
    def test_interface_check_func_without_args(self):
        s_func = self.v.stub_method(sc, 'simple_func')
        try:
            s_func(g=5)
            self.fail()
    ...
    def test_interface_check_func_with_default_value(self):
        s_func = self.v.stub_method(sc, 'func_with_defaults')
        s_func(1)
        s_func(1, 2)
        s_func(a=1, b=2)
        try:
    ...
    def test_intefrace_check_func_with_one_arg(self):
        s_func = self.v.stub_method(sc, 'func_with_one_arg')
        try:
            s_func(1, g=5)
            self.fail()
        except mockerrors.InterfaceError:
    ...
    def test_intefrace_check_func_with_one_arg_and_many_other_args(self):
        s_func = self.v.stub_method(sc, 'func_with_one_arg_and_many')
        s_func(1, 5, 6)
        try:
            s_func(1, 5, 6, g=5)
            self.fail()
    ...
    def test_handled_error_regenerated(self):
        other_func = self.v.mock_method(sc, 'func_with_one_arg')
        s_func = self.v.mock_method(sc, 'simple_func')
        s_func()
        self.v.replay()
        try:
            other_func(123)
